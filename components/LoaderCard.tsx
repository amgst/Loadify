...
const LoaderCard: React.FC<LoaderCardProps> = ({ loader, config, onViewCode }) => {
  ...
  return (
    <div className="group relative bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 hover:border-slate-600 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-blue-900/10 hover:-translate-y-1">
      <div 
      ...
      >
        <div ref={containerRef} dangerouslySetInnerHTML={{ __html: loader.html }} />
        ...
        {loader.isAiGenerated && (
            <span className="absolute top-3 right-3 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-purple-500/20 text-purple-300 border border-purple-500/30 rounded-full">
                AI Generated
            </span>
        )}
      </div>
