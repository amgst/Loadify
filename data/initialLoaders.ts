...
const createSpinner = (id: string, name: string, css: string, html: string = '<div class="loader"></div>'): LoaderItem => ({
  id, name, type: 'spinner', html: html.replace('class="loader"', `class="loader-${id}"`), css: css.replace('.loader', `.loader-${id}`)
});
...
const createDots = (id: string, name: string, css: string, count: number = 3): LoaderItem => {
  ...
  return {
    id, name, type: 'dots',
    html: `<div class="loader-${id}">${divs}</div>`,
    css: css.replace('.loader', `.loader-${id}`)
  };
};
