...
exports.initialLoaders = void 0;
const loaderClassRegex = /\.loader/g;
const buildLoaderHtml = (html, id) => {
...
const scopeLoaderCss = (css, id) => css.replace(loaderClassRegex, `.loader-${id}`);
const createSpinner = (id, name, css, html = '<div class="loader"></div>') => ({
    id,
    name,
    type: 'spinner',
...
const createDots = (id, name, css, count = 3) => {
...
const createBar = (id, name, css, count = 5) => {
    const divs = Array(count).fill('<div></div>').join('');
    return {
        id, name, type: 'bar',
        html: buildLoaderHtml(`<div class="loader">${divs}</div>`, id),
...
.loader { display: flex; gap: 8px; align-items: center; justify-content: center; width: var(--loader-size); }
...
.loader { display: flex; gap: 4px; justify-content: center; align-items: center; width: var(--loader-size); }
...
.loader { display: flex; gap: 6px; align-items: center; width: var(--loader-size); }
...
.loader { display: flex; gap: 8px; justify-content: center; width: var(--loader-size); }
...
.loader { display: flex; justify-content: space-between; width: var(--loader-size); }
